...
impl Network {
    ...
    // Здесь была `from_rpc_url`: сеть угадывалась по слову «devnet» в адресе
    // узла, а всё прочее считалось mainnet. Её никто не звал, и хорошо — такой
    // дефолт при любой неузнанной форме адреса (свой узел, прокси, провайдер
    // без «devnet» в имени) молча выбирает сеть с настоящими деньгами. Сеть
    // задаётся явно — moniker'ом из конфига, — а если однажды понадобится
    // спросить её у самой цепочки, ответ даёт genesis hash, а не имя хоста.
