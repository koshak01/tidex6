...
- `programs/tidex6-tip-jar/` (deployed and OtterSec-verified on mainnet in April 2026 at [`5WohQRRzC31SkFMSWgEqJC9p2KvNhGkQbzUSsNUi9b9x`](https://solscan.io/account/5WohQRRzC31SkFMSWgEqJC9p2KvNhGkQbzUSsNUi9b9x); the demo deployment has since been closed — the program data account is closed and the rent reclaimed, so the program no longer executes. The source and the verification record remain) — third-party Anchor program that uses `tidex6_verifier::deposit` via CPI. Demonstrates that any Solana protocol — DAO payroll, NFT royalty splitter, subscription protocol, dark-pool DEX hook — can adopt tidex6 as a privacy primitive in ~30 lines of Rust.
