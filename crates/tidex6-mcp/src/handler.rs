...
impl Tidex6Mcp {
    ...
    async fn ceremony(&self) -> Result<CallToolResult, McpError> {
        ...
        // Query param must stay on the URL: session return path + anti-sybil attribution.
        // Put URL on line 1 so hosts that truncate or paraphrase still keep ?s=.
        ...
        // Short on purpose: long essays get paraphrased and lose ?s= (seen in Grok).
        let text = format!(
            "CONTRIBUTE_URL: {contribute_url}\n\
             NONCE: {nonce}\n\
             MCP: tidex6-mcp {version}\n\n\
        ...
        let mut result = CallToolResult::success(vec![ContentBlock::text(text)]);
        // Structured first-class for agents that prefer JSON over prose.
        result.structured_content = Some(serde_json::json!({
            "contributions": total,
